...
mean.median <- summarize(daily.sum, daily.step.mean = mean(daily.total.steps, na.rm = TRUE),
          daily.step.median = median(daily.total.steps))
```
As we can see, the mean was `r mean.median$daily.step.mean` and median was `r mean.median$daily.step.median`.
...
From the following result, we can see the maximum number `r max(interval.mean$mean.steps)` was recorded at the interval `r interval.mean[which.max(interval.mean$mean.steps), "interval"]` (08:35).
```{r}
...
new.mean.median <- summarize(daily.imputed.sum, daily.step.mean = mean(daily.total.steps, na.rm = TRUE),
          daily.step.median = median(daily.total.steps))
```
The new mean was `r new.mean.median$daily.step.mean` and the new median was `r new.mean.median$daily.step.median`. These were almost the same as the ones obtained with the data before imputation in Part II. 2.
